...
import iminuit
def function(omega, a, b, c):
    function = a*(1/(1-2*b*omega)+c)
    return function

def chi2(a, b, c):
    chi2 = np.sum(((sin2beta_width_array_cfg - function(omega_array_cfg, a, b, c))/sin2beta_width_error_array_cfg)**2)
    return chi2
...
ax.set_ylabel(r"Standard Deviation $\sigma(\sin(2 \beta))$")
ax.errorbar(dilution_array_cfg, sin2beta_width_array_cfg, yerr=sin2beta_width_error_array_cfg, fmt="o", markersize=4, capsize=2, color="blue", label="Simulation Result")
#ax.errorbar(dilution_array_cfg, S_f_width_expected_array_cfg, yerr=S_f_width_error_expected_array_cfg, fmt="o", markersize=4, capsize=2, color="red", label="Fisher Information Prediction")
ax.errorbar(dilution_array_cfg, S_f_width_expected_array_cfg, fmt="o", markersize=4, color="red", label="Fisher Information Prediction")
ax.plot(dilution_array_cfg, a*(1/(1-2*b*omega_array_cfg)+c), color="black")
ax.plot(dilution_array_cfg, S_f_width_expected_array_cfg, color="black", linestyle="dashed")
ax.set_xlim(dilution_array_cfg[-1]-0.02, 1)
ax.set_ylim(0, )
ax.legend(loc="upper right")
ax.grid(True)
plt.tight_layout()
plt.savefig("sin2beta_width_vs_dilution_fisher_information.pdf")
plt.show()